...
LRESULT WINAPI MelterProc(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
    switch(Msg){
        ...
        case WM_TIMER:
        {
            ...
            int nYPos = (rand() % nRandHeight);
            int nWidth = (rand() % nRandWidth);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrev, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    if(!RegisterClass(&wndClass))
        return MessageBox(HWND_DESKTOP, "Cannot register class!",
                          NULL, MB_ICONERROR | MB_OK);

    HWND hWnd = CreateWindow("Melter", NULL, WS_POPUP, 0, 0,
                             nScreenWidth, nScreenHeight, HWND_DESKTOP, NULL,
                             hInstance, NULL);
    if(!hWnd)
        return MessageBox(HWND_DESKTOP, "Cannot create window!", NULL,
                          MB_ICONERROR | MB_OK);
